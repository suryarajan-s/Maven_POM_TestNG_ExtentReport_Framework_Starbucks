...
public class Delivery_Transactions_3 extends BaseTest {
	...
	public void DeliveryUberEats() throws IOException, InterruptedException {
		...
		try {
			loginPage.verifyLogin(testData.getTestData("3_TR", "expectedUserName"));
		...
		try {
			homePage.selectStoreAndDeliveyAddress(testData.getTestData("3_TR", "streetName"));
		...
		try {
			homePage.selectProducts(testData.getTestData("3_TR", "menuData"), testData.getTestData("3_TR", "productName"));
		...
		try {
			homePage.customizeNonDrinkProduct(testData.getTestData("3_TR", "quantityValueDropDown"));
			extentTestThread.get().log(Status.PASS,
		...
		try {
			orderPlacepage.addToOrderForNonDrink("3_TR");
			extentTestThread.get().log(Status.PASS,
		...
		try {
			orderPlacepage.editDeliveryOption(testData.getTestData("3_TR", "editDeliveryOption"),testData.getTestData("3_TR", "dropOffOption"));
			extentTestThread.get().log(Status.PASS,
					"We have edited delivery options and validated as per user data and proceeded with the checkout In the UberEats App");
			Thread.sleep(4000);
		} catch (Throwable t) {
			extentTestThread.get().log(Status.FAIL,
					"Error in editing delivery options  validating and checking out in Uber eats: " + t.getMessage());
			throw t;
		}


		try {
			orderPlacepage.placeOrder("3_TR");
		...
		try {
			rALoginPage.rALogin(testData.getTestData("3_TR", "rAUserName"), testData.getTestData("3_TR", "rACompanyName"),
					testData.getTestData("3_TR", "rAPassword"));
		...
		try {
			guestCheckPage.rAGuestCheckReport(testData.getTestData("3_TR", "rAStoreName"),
					testData.getTestData("3_TR", "productName"), testData.getTestData("3_TR", "productPrize"),
					"3_TR");
